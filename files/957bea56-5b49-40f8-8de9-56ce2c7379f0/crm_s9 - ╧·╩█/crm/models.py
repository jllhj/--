...
class ClassList(models.Model):#班级表
    ...
    teachers=models.ManyToManyField(verbose_name="任课老师",to="UserInfo",related_name="abc",limit_choices_to={"depart__in":[1002,1005]})  # limit_choices_to 过滤数据  相当于objects.filter(depart__in=)
...
class Customer(models.Model):#客户表
        ...
        work_status=models.IntegerField(verbose_name="在职状态",choices=work_status_choices,default=1,blank=True,
                                        null=True)
        ...
        consultant=models.ForeignKey(verbose_name="课程顾问",to="UserInfo",related_name="consult",
        ...
        def  __str__(self):
            # return f"姓名:{self.name},QQ:{self.qq}"
            return self.name
...
class ConsultRecord(models.Model):#客户跟进记录
    ...
    consultant=models.ForeignKey(verbose_name="跟踪人",to="UserInfo",on_delete=models.CASCADE,limit_choices_to={"depart__in":[1001]})
    date=models.DateField(verbose_name="跟进日期",auto_now_add=True)
    note=models.TextField(verbose_name="跟进内容")

    def __str__(self):
        return f"{self.customer.name}:{self.consultant.name}"

class PaymentRecord(models.Model):#缴费记录
    ...
    consultant=models.ForeignKey(verbose_name="负责老师",to="UserInfo",help_text="谁签的单",on_delete=models.CASCADE)

    def __str__(self):
        return f"{self.customer} {self.pay_type}"

class Student(models.Model):#学生表(已报名)
    customer=models.OneToOneField(verbose_name="客户信息",to="Customer",on_delete=models.CASCADE)
    username=models.CharField(verbose_name="用户名",max_length=32)
    password=models.CharField(verbose_name="密码",max_length=64)
    emergency_contract=models.CharField(verbose_name="紧急联系人",max_length=32,blank=True,null=True)
    ...
    date=models.DateField(verbose_name="入职时间",help_text="格式:年-月-日",blank=True,null=True)
    memo=models.CharField(verbose_name="备注",max_length=256,blank=True,null=True)

    def __str__(self):
        return self.username

class CourseRecord(models.Model):#上课记录表
    class_obj=models.ForeignKey(verbose_name="班级",to="ClassList",on_delete=models.CASCADE)
    day_num=models.IntegerField(verbose_name="节次",help_text=u"此处填写第几节课或第几天课程")
    teacher=models.ForeignKey(verbose_name="讲师",to='UserInfo',limit_choices_to={"depart__in":[1002,1005]},on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.class_obj} day{self.day_num}"

class StudyRecord(models.Model):#学生记录表
    ...
    def __str__(self):
        return f"{self.course_record}-{self.student}"

class CustomerDistrbute(models.Model):#客户分布表
    customer=models.ForeignKey(to="Customer",related_name="customers",verbose_name="客户",on_delete=models.CASCADE)
    ...
    status=models.IntegerField(choices=status_list,default=1,verbose_name="状态")
    memo=models.CharField(max_length=255,verbose_name="说明")
    def __str__(self):
        return self.customer.name+"的课程顾问是:"+self.consultant.name